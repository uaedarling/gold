...
def init_csv_files():
    ...
    if not os.path.exists(SPREADS_FILE):
        with open(SPREADS_FILE, "w", newline="") as f:
            csv.writer(f).writerow([
                "datetime","spread","spread_limit","session"
            ])
...
def save_spread_snapshot(spread, limit, session):
    with open(SPREADS_FILE, "a", newline="") as f:
        csv.writer(f).writerow([
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            spread, limit, session
        ])
#############################################
# LOGGING
#############################################
...
def spread_ok():
    spread  = get_spread()
    session = get_session()
    limit   = SPREAD_LIMITS.get(session, 20.0)
    if spread > limit:
        log(f"SPREAD BLOCKED | {spread} > max {limit} ({session})")
        return False
    return True
...
def dashboard(tf_data, buy_score, sell_score, spread, signal_type, current_atr):
    ...
    print("=" * 64)
    print(col("             GOLD BOT V23 — LIVE DASHBOARD", Fore.YELLOW if COLOR else ""))
    print("=" * 64)
    ...
    spread_limit = SPREAD_LIMITS.get(session, 20.0)
    print(f"  ATR(M5)  : {col(str(atr), atr_col)}"
          f"  (baseline: {round(atr_baseline,2) if atr_baseline else 'calc...'})"
          f"  |  Spread: "
          f"{col(str(spread), (Fore.GREEN if COLOR else '') if spread <= spread_limit else (Fore.RED if COLOR else ''))}"
          f"  (max {spread_limit})")
...
def open_trade(direction, atr, price, ema200,
    ...
    if not spread_ok():
        ...
        reason = f"SPREAD={spread_val}"
        log_missed_opportunity(direction, buy_score, sell_score,
                               signal_type, reason, tf_data)
        save_signal(direction, buy_score, sell_score, signal_type,
                    False, reason, price, spread_val, get_session(), tf_data)
        return
    ...
    request = {
        ...
        "deviation":    20,
        "magic":        MAGIC,
        "comment":      f"GoldBotV23_{signal_type[:3]}",
...
def generate_signal():
    ...
    # Save spread snapshot every cycle
    session = get_session()
    save_spread_snapshot(spread, SPREAD_LIMITS.get(session, 20.0), session)
...
log(f"Spread limits: Asia={SPREAD_LIMITS['ASIA']} | London={SPREAD_LIMITS['LONDON']} | NY={SPREAD_LIMITS['NEW YORK']}")
log(f"Signal queue TTL=60s (retry on spread normalization)")
